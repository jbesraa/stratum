...
use network_helpers_sv2::noise_with_tokio_channels::SV2ConnectionSetup;
use roles_logic_sv2::{
    parsers::{
...
impl Sniffer {
    ...
    async fn create_downstream(
        ...
        if let Ok((receiver_from_client, sender_to_client)) =
            SV2ConnectionSetup::new(HandshakeRole::Responder(responder))
                .start(stream)
                .await
        {
    ...
    async fn create_upstream(
        ...
        if let Ok((receiver_from_server, sender_to_server)) =
            SV2ConnectionSetup::new(HandshakeRole::Initiator(initiator))
                .start(stream)
                .await
        {
    ...
    async fn recv_from_down_send_to_up(
        mut recv: Receiver<MessageFrame>,
...
#[macro_export]
macro_rules! assert_message_tokio {
...
#[macro_export]
macro_rules! assert_common_message_tokio {
  ($msg:expr, $expected_message_variant:ident, $($expected_property:ident, $expected_property_value:expr),*) => {
	assert_message_tokio!(Common, CommonMessages, $msg, $expected_message_variant, $($expected_property, $expected_property_value),*);
  };
  ($msg:expr, $expected_message_variant:ident) => {
	assert_message_tokio!(Common, CommonMessages, $msg, $expected_message_variant);
  };
}
...
// properties and values.
#[macro_export]
macro_rules! assert_tp_message_tokio {
  ($msg:expr, $expected_message_variant:ident, $($expected_property:ident, $expected_property_value:expr),*) => {
	assert_message_tokio!(TemplateDistribution, TemplateDistribution, $msg, $expected_message_variant, $($expected_property, $expected_property_value),*);
  };
  ($msg:expr, $expected_message_variant:ident) => {
	assert_message_tokio!(TemplateDistribution, TemplateDistribution, $msg, $expected_message_variant);
  };
}

// Assert that the message is a mining message and that it has the expected properties and values.
#[macro_export]
macro_rules! assert_mining_message_tokio {
  ($msg:expr, $expected_message_variant:ident, $($expected_property:ident, $expected_property_value:expr),*) => {
	assert_message_tokio!(Mining, Mining, $msg, $expected_message_variant, $($expected_property, $expected_property_value),*);
  };
  ($msg:expr, $expected_message_variant:ident) => {
	assert_message_tokio!(Mining, Mining, $msg, $expected_message_variant);
  };
}
...
// values.
#[macro_export]
macro_rules! assert_jd_message_tokio {
  ($msg:expr, $expected_message_variant:ident, $($expected_property:ident, $expected_property_value:expr),*) => {
	assert_message_tokio!(JobDeclaration, JobDeclaration, $msg, $expected_message_variant, $($expected_property, $expected_property_value),*);
  };
  ($msg:expr, $expected_message_variant:ident) => {
	assert_message_tokio!(JobDeclaration, JobDeclaration, $msg, $expected_message_variant);
  };
}
